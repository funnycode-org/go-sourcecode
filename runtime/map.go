...
const (
	...
	bucketCntBits = 3
	bucketCnt     = 1 << bucketCntBits // 每个bmap里面存8个值 1000
	...
	// flags
	iterator     = 1 // there may be an iterator using buckets 1
	oldIterator  = 2 // there may be an iterator using oldbuckets 10
	hashWriting  = 4 // a goroutine is writing to the map 100
...
func (h *hmap) newoverflow(t *maptype, b *bmap) *bmap {
	...
	if h.extra != nil && h.extra.nextOverflow != nil {
		...
		if overflowBmap == nil { // 该桶没有逸出桶
			...
			// 该桶的溢出桶都被使用了，那么也就表示你预先申请的桶都使用完了，我们的目的是先使用完预先申请的桶
			// 已经用完了，就把nextOverflow相关信息清除
	...
	// 拿一个事先申请好的逸出桶作为当面bmap的逸出桶
	// 在不断的给map添加新值的时候，会不断的用该bmap的逸出桶，也就是会使用事先申请好的hmap的逸出桶
	b.setoverflow(t, ovf)
	return ovf
}
